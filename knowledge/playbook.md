...
**Post:** cache-bust `?v=` en `redesign.css/js`, vaciar `app/templates/.cache/*` (⚠ contenido, NO el dir raíz — ver §16), hard refresh. Verificar dashboard + navegación entre iframes con el tema. Es rediseño por capa → reversible (quitar el link/js vuelve al theme original).
...
**Twig:** brand/links internos → `href="."` (resuelve contra `{{ base_assets()|raw }}` = `<base href>` ya presente). Tras editar, **vaciar `app/templates/.cache/*`** (twig compilado viejo lleva el dominio; regenera. ⚠ contenido, no el dir raíz — ver §16).
...
- **"El fix de JS no surte efecto" = JS cacheado sin versión:** los `<script src="...x.js">` de Ocrend se incluyen **sin `?v=`** → el browser cachea el JS viejo indefinidamente y sigue corriendo el código previo aunque el archivo en disco esté arreglado. Síntoma: editás el .js, el server hace lo correcto, pero el usuario "no ve el cambio". Fix inmediato: hard refresh (Ctrl+Shift+R). Fix permanente: agregar cache-bust al include (`src="....js?v=YYYYMMDDx"`) y **vaciar `app/templates/.cache/*`** para que el Twig recompile con el nuevo tag (⚠ contenido, no el dir raíz — borrarlo entero da 500, ver §16). Validado en erp__Fasmot (`expedientes.twig` → `__x__entradas_v03.js`).
- **Recorrer array vacío/null = Warning → 500 en PHP 8 (y TypeError en JS):** `foreach` sobre `null`/`false` (Ocrend `select()` retorna `false` sin filas) → `Warning: foreach() argument must be of type array` → 500 con `debug:true`. En JS, `for(i=0;i<x.length;i++)` o `x.forEach()` sobre `undefined` → `Cannot read properties of undefined`. Fix MECÁNICO SEGURO (no cambia lógica si ya hay datos):
  - **PHP foreach**: `foreach ($x as $v)` → `foreach ((is_array($x) ? $x : []) as $v)`. **Solo** sobre variable simple (`$x`, `$x['k']`); NO sobre `foreach(funcion() as` (la envoltura ejecutaría la función 2 veces) ni `as &$ref` (rompe la referencia — itera copia). NO usar `(array)$x` (¡`(array)false` = `[false]`, itera 1 vez!). Idempotente (no re-envolver los que ya tienen `is_array`).
  - **JS for/length**: `i < x.length` → `i < (x || []).length` (vale para array y string; null/undefined → 0).
  - **JS forEach**: `x.forEach(` → `(x || []).forEach(` (solo receptor identificador/props, no llamadas).
  - Validado en erp__Fasmot: 457 foreach PHP (134 archivos) + 687 for/length + 18 forEach JS, 0 lint/parse fail.
  - **No cubierto (requiere criterio, reportar)**: `$x[0]['k']` post-`select()` sin `false !== $x` (no es foreach), `foreach` sobre llamadas o `&$ref`, `for...of`/`.map`/`.filter` JS sobre posible undefined.
...
- **Descarga de archivos del MISMO origen: usar `<a href download>` directo, NO fetch+blob.** Para archivos que el server ya escribió en disco y sirve por URL (mismo origen), el método correcto es `<a href=encodeURI(ruta) download=nombre>` + `click()` → el browser pide el archivo al server y lo guarda. **No usar `fetch()+blob`** salvo necesidad real (cross-origin con CORS, o transformar el contenido): introduce caché HTTP del fetch (la 2ª descarga sirve respuesta cacheada/`304` sin cuerpo → "solo descarga la 1ª vez tras limpiar caché"), consumo de memoria con archivos grandes, y problemas de resolución de ruta relativa dentro de iframes. Si SÍ se usa blob: cache-bust `?_t=Date.now()` + `{cache:'no-store'}` + revoke DIFERIDO (`setTimeout(()=>revokeObjectURL(url),4000)`; revoke inmediato cancela la descarga) + validar `blob.size>0`. Bug colateral: `revokeObjectURL(element)` recibiendo el `<a>` en vez de la `url`.
- **Descargas múltiples automáticas: solo baja la 1ª.** Los browsers bloquean descargas programáticas que van muy juntas. Fix: ejecutarlas **en secuencia con delay ~1200ms entre cada una** (no en paralelo). La 1ª vez el browser puede preguntar "¿permitir descargas múltiples?" → aceptar. Validado en erp__Fasmot (R_Save_ExceL: xlsx + word/SEI → `descargarSecuencia()` con `<a download>` directo).
